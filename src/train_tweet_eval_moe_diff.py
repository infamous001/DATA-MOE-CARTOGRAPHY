...
from utils.helper1 import CustomModel, ModelArgs, MoeArgs
from utils.helper1 import ConfiguredMetric
...
with open("/home/pritam.k/research/data-moe/results/best_hyperparameters_tweet_eval.json", "r") as jfile:
...
mode=f"{num_experts}_cbz_diff_final" #ce/cbz/cb/cz/15_3_cbz


args=ModelArgs(dim=768,hidden_dim=3072,num_labels=3,moe=MoeArgs(num_experts,num_experts_per_tok,3))
...
ds_train = load_dataset('csv', data_files='/home/pritam.k/research/data-moe/src/MOE-DIFF_data/tweet_eval_train_diff.csv')
ds_val = load_dataset('csv', data_files='/home/pritam.k/research/data-moe/src/MOE-DIFF_data/tweet_eval_val_diff.csv')
ds_test = load_dataset('csv', data_files='/home/pritam.k/research/data-moe/src/MOE-DIFF_data/tweet_eval_test_diff.csv')
...
for epoch in range(num_epochs):
    ...
    for batch in train_dataloader:
        e = 'global_index'
        f = 'difficulty'
        batch_d={k: v.to(device) for k, v in batch.items() if k == f }
        batch = {k: v.to(device) for k, v in batch.items() if k not in (e, f)}
        outputs, selected_experts = model(**batch,difficulty_label=torch.tensor(batch_d[f]))
        loss = outputs.loss
...
plt.legend()
plt.savefig(f'plots/Loss_{mode}_tweet_eval.png')
plt.show()
...
for batch in test_dataloader:
    ...
    batch = {k: v.to(device) for k, v in batch.items() if k not in (e, f)}
    with torch.no_grad():
        outputs, s = model(**batch,difficulty_label=torch.tensor(batch_d[f]))
        for k in s.keys():
