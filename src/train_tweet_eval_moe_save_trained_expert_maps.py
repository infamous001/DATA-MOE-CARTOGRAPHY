...
num_epochs=1 #hp_params['num_train_epochs']
num_experts=3
num_experts_per_tok=1
mode=f"{num_experts}_{num_experts_per_tok}_cbz" #ce/cbz/cb/cz/
...
val_losses = []   
# expert_list_train=[]
# expert_list_train = {epoch:{key:[] for key in range(num_experts)} for epoch in range(num_epochs)}
expert_list_train = {epoch:{} for epoch in range(num_epochs)}
for epoch in range(num_epochs):
    ...
    for batch in train_dataloader:
        ...
        expert_maps.append(selected_experts)
        loss = outputs.loss
    ...
    expert_list_train[epoch].update(expert_maps)
...
with open(f"results/tweet_eval_{mode}_train_expert_mappings.json", "w") as jfile:
    json.dump(expert_list_train, jfile)
...
with open(f"results/scores_tweet_eval_{mode}_temp.json", "w") as jfile:
...
with open(f"results/expert_dist_{mode}_tweet_eval_temp.json", "w") as jfile:
...
with open(f"results/expert_mapped_{mode}_tweet_eval_temp.json", "w") as jfile:
