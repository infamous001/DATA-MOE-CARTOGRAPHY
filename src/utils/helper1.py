...
class MoeArgs:
  def __init__(self, num_experts: int,num_experts_per_tok:int,num_difficulty_labels:int):
    self.num_experts=num_experts
    self.num_experts_per_tok=num_experts_per_tok
    self.num_difficulty_labels=num_difficulty_labels
...
class MoeLayer(nn.Module):
    def __init__(self, experts: List[nn.Module], gate: nn.Module, moe_args: MoeArgs):
        ...
        self.args = moe_args
        self.difficulty_embedding = nn.Embedding(self.args.num_difficulty_labels, 768)
        #self.expert_assignments = None

    def forward(self, inputs: torch.Tensor,difficulty_labels):
        difficulty_labels_em=self.difficulty_embedding(difficulty_labels)
        inputs_final=torch.cat([inputs,difficulty_labels_em],dim=1)
        gate_logits = self.gate(inputs_final)
...
class CustomModel(nn.Module):
  def __init__(self,checkpoint,args: ModelArgs):
    ...
    self.args=args
    self.moe_layer=MoeLayer(experts=[FeedForward(args=args) for _ in range(args.moe.num_experts)],gate=nn.Linear(2*args.dim, args.moe.num_experts, bias=False),moe_args=args.moe,)
    self.classifier = nn.Linear(768,args.num_labels)

  def forward(self, input_ids=None, attention_mask=None,labels=None,difficulty_label=None):
    outputs = self.model(input_ids=input_ids, attention_mask=attention_mask)
    moe_outputs,loss_z,loss_b,expert_tracking = self.moe_layer(outputs[0][:,0,:].view(-1,768),difficulty_label)
