...
class MoeLayer(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        return results, loss_z, loss_b, expert_tracking,gate_logits
...
class CustomModel(nn.Module):
  def __init__(self,checkpoint,args: ModelArgs):
    ...
    self.classifier = nn.Linear(768,args.num_labels*3)

  def forward(self, expert_label,input_ids=None, attention_mask=None,labels=None):
    outputs = self.model(input_ids=input_ids, attention_mask=attention_mask)
    moe_outputs,loss_z,loss_b,expert_tracking,gate_logits = self.moe_layer(outputs[0][:,0,:].view(-1,768))
    ...
    if labels is not None:
      ...
      loss_logits = loss_fct(logits.view(-1, self.args.num_labels*3), labels.view(-1))
      loss_expert=loss_fct(gate_logits.view(-1,3),expert_label.view(-1))
      values=torch.tensor([loss_b,loss_z,loss_expert,loss_logits])
      normalized_values = (values - values.min()) / (values.max() - values.min())
      loss=normalized_values.sum()
