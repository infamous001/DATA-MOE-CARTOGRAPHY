...
from utils.helper_expr import CustomModel, ModelArgs, MoeArgs
from utils.helper_expr import ConfiguredMetric
...
mode=f"{num_experts}_{num_experts_per_tok}_exp1" #ce/cbz/cb/cz/15_3_cbz
dt="tweet_eval"
...
ds_train = load_dataset('csv', data_files=f'data/{dt}/m_train.csv')
ds_val = load_dataset('csv', data_files=f'data/{dt}/m_val.csv')
ds_test = load_dataset('csv', data_files=f'data/{dt}/m_test.csv')
...
tokenized_dataset.set_format("torch",columns=["global_idx","input_ids", "attention_mask", "labels","diff"])
...
for epoch in range(num_epochs):
    ...
    for batch in train_dataloader:
        e = 'global_idx'
        f="diff"
        expert_label = [v for k, v in batch.items() if k == f]
        if len(expert_label) == 1:  # Ensure it is not a list of lists
            expert_label = expert_label[0]  # Extract the first (and only) element
        expert_label = expert_label.to(device)
        batch = {k: v.to(device) for k, v in batch.items() if (k != e and k!=f)}
        outputs, selected_experts = model(expert_label=expert_label,**batch)
        loss = outputs.loss
    ...
    for batch in eval_dataloader:
        ...
        batch = {k: v.to(device) for k, v in batch.items() if (k != e and k!=f)}
        with torch.no_grad():
            outputs, sel = model(expert_label=expert_label,**batch)
...
for batch in test_dataloader:
    ...
    batch = {k: v.to(device) for k, v in batch.items() if (k != key_to_exclude and k!=f)}
    with torch.no_grad():
        outputs, s = model(expert_label=expert_label,**batch)
        for k in s.keys():
